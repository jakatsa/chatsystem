...
class ConversationListCreateView(generics.ListCreateAPIView):
    ...
    def create(self,request, *args,**kwargs):
        ...
        if len(participants_data) !=2:
            return Response(
                {'error':'A convo only needs two participants'},
                status=status.HTTP_400_BAD_REQUEST
            )
        
        if str(request.user.id) not in map(str,participants_data):
            return Response(
                {'error':'You arent a participant of this convo'},
                status=status.HTTP_403_FORBIDDEN
            )
        users = User.objects.filter(id__in=participants_data)
        if users.count() !=2:
            return Response(
                {'error':'A convo needs 2 participants'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if existing_conversation.exists():
            return Response({
                'error':'A convo already exists between these participants'
            },
            status=status.HTTP_400_BAD_REQUEST
